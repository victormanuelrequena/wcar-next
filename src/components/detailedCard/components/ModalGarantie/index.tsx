...
export default function ModalGarantie({
  close,
  id,
  carValue,
  SrvCode,
  ...
  const bookCar = async () => {
    ...
    console.log({
      ...
      totalToPay: parseFloat(
        formatValue(options[security].value.toString().trimStart())
      ),
    });
    try {
      const response = await window.fetch(
        "https://api.wcaronline.com/api/garantie-security/",
        {
      ...
      <div className="closer">
        <div className="modal-amount position-relative">
          ...
          <h5 className="modal-title">
            Adquiere tu {SrvCode == "1002" ? "Seguro" : "Garantia"}
          </h5>
