    ...
    const onChangeFunction = (event: any) => {
      ...
      if (price) {
        ...
        if (value !== "") {
          ...
          selectionStart =
            selectionStart + value.length - inputElement.value.length;
          selectionEnd =
            selectionEnd + value.length - inputElement.value.length;
        }
      ...
      validateInside = {
        ...
        minPriceValue: (value: string) => {
          ...
          return _value >= minValue
            ? null
            : `Debe ser mayor o igual a ${new Intl.NumberFormat("es-CO", {
                style: "currency",
                currency: "COP",
              })
                .format(minValue)
    ...
    if (price) {
      validateInside = {
        ...
        maxPriceValue: (value: string) => {
          const _value = priceToInt(value);
          return _value <= maxValue * 10
            ? null
            : `Debe ser menor o igual a ${new Intl.NumberFormat("es-CO", {
                style: "currency",
                currency: "COP",
              })
                .format(maxValue)
  ...
  if (noUpperCase) {
    validateInside = {
      ...
      noUpperCase: (value: string) =>
        value.match(/^((?![A-Z]).)*$/g)
          ? "No se permiten letras mayúsculas"
          : null,
    };
  }
  ...
  if (noLowerCase) {
    validateInside = {
      ...
      noLowerCase: (value: string) =>
        value.match(/^((?![a-z]).)*$/g)
          ? "No se permiten letras minúsculas"
          : null,
    };
  }
